  ...
  return (
    <div className={`technique-row${covered ? ` covered coverage-${coverage}` : ''}`}>
      ...
      {coverage && coverage !== 'not-covered' && (
        <span
          className={`coverage-dot coverage-dot--${coverage}`}
          title={`${COVERAGE_LABELS[coverage]}${note ? ': ' + note : ''}`}
        />
      )}
            ...
            return (
              <div key={sub.id} className={`sub-row${subCovered ? ` covered coverage-${subCoverage}` : ''}`}>
                ...
                {subCoverage && subCoverage !== 'not-covered' && (
                  <span
                    className={`coverage-dot coverage-dot--${subCoverage}`}
                    title={`${COVERAGE_LABELS[subCoverage]}${subNote ? ': ' + subNote : ''}`}
                  />
                )}
              </div>
            )
